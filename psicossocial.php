...
$consulta = select("atendimento","*","WHERE psicossocial LIKE 'A' AND dia LIKE '%$dia%' AND mes LIKE '%$mes%' AND ano LIKE '%$ano%'","ORDER BY id ASC");
$consulta2 = select("atendimento","*","WHERE psicossocial LIKE 'O' AND dia LIKE '%$dia%' AND mes LIKE '%$mes%' AND ano LIKE '%$ano%'","ORDER BY id ASC");
?>
...
<strong>RELATÓRIO DE ATENDIMENTO - AVALIAÇÃO PSICOLOGICA</strong>
</div>
  ...
  <div class="col-md-12">
  	<div class="panel panel-default">
        ...
        <?php
        $clinico = $pdo->query("SELECT * FROM atendimento WHERE psicossocial LIKE 'A'  AND dia LIKE '%$dia%' AND mes LIKE '%$mes%' AND ano LIKE '%$ano%' ORDER BY id")->fetchAll();
        $t_clinico = count($clinico);
        ?>	             
	     	Total de colaboradores - Avaliação Psicologica: <strong><?php print $t_clinico; ?></strong>               
	   	</div>
  	</div>
 	</div>
  ...
  <ul class="list-group">
    ...
    <a href ="#" onclick="window.open('psicossocial_ok.php?id=<?php echo $consulta[$i]['id'] ?>', 'Pagina', 'STATUS=NO, TOOLBAR=NO, LOCATION=NO, DIRECTORIES=NO, RESISABLE=NO, SCROLLBARS=YES, TOP=50, LEFT=50, WIDTH=550, HEIGHT=400');">
