...
$consulta = select("atendimento","*","WHERE eeg LIKE 'A' AND dia LIKE '%$dia%' AND mes LIKE '%$mes%' AND ano LIKE '%$ano%'","ORDER BY id ASC");
$consulta2 = select("atendimento","*","WHERE eeg LIKE 'O' AND dia LIKE '%$dia%' AND mes LIKE '%$mes%' AND ano LIKE '%$ano%'","ORDER BY id ASC");
...
<html lang="pt-BR">
  <head>
    ...
    <meta http-equiv="refresh" content="10;URL=eeg.php">
    <!-- The above 3 meta tags *must* come first in the head; any other head content must come *after* these tags -->
    
    <title>EEG | SALA ONLINE | COPREVEN</title>
...
<strong>RELATÓRIO DE ATENDIMENTO - ELETROENCEFALOGRAMA</strong>
</div>
  ...
  <div class="col-md-12">
  	<div class="panel panel-default">
        ...
        <?php
        $clinico = $pdo->query("SELECT * FROM atendimento WHERE eeg LIKE 'A'  AND dia LIKE '%$dia%' AND mes LIKE '%$mes%' AND ano LIKE '%$ano%' ORDER BY id")->fetchAll();
        $t_clinico = count($clinico);
        ?>	             
	     	Total de colaboradores - Eletroencafalograma: <strong><?php print $t_clinico; ?></strong>               
	   	</div>
  	</div>
 	</div>
  ...
  <ul class="list-group">
    ...
    <a href ="#" onclick="window.open('eeg_ok.php?id=<?php echo $consulta[$i]['id'] ?>', 'Pagina', 'STATUS=NO, TOOLBAR=NO, LOCATION=NO, DIRECTORIES=NO, RESISABLE=NO, SCROLLBARS=YES, TOP=50, LEFT=50, WIDTH=550, HEIGHT=400');">
