...
$consulta = select("atendimento","*","WHERE ecg LIKE 'A' AND dia LIKE '%$dia%' AND mes LIKE '%$mes%' AND ano LIKE '%$ano%'","ORDER BY id ASC");
$consulta2 = select("atendimento","*","WHERE ecg LIKE 'O' AND dia LIKE '%$dia%' AND mes LIKE '%$mes%' AND ano LIKE '%$ano%'","ORDER BY id ASC");
...
<html lang="pt-BR">
  <head>
    ...
    <meta http-equiv="refresh" content="10;URL=ecg.php">
    <!-- The above 3 meta tags *must* come first in the head; any other head content must come *after* these tags -->
    
    <title>ECG | SALA ONLINE | COPREVEN</title>
...
<strong>RELATÓRIO DE ATENDIMENTO - ELETROCARDIOGRAMA</strong>
</div>
  ...
  <div class="col-md-12">
  	<div class="panel panel-default">
        ...
        <?php
        $clinico = $pdo->query("SELECT * FROM atendimento WHERE ecg LIKE 'A'  AND dia LIKE '%$dia%' AND mes LIKE '%$mes%' AND ano LIKE '%$ano%' ORDER BY id")->fetchAll();
        $t_clinico = count($clinico);
        ?>	             
	     	Total de colaboradores - Eletrocardiograma: <strong><?php print $t_clinico; ?></strong>               
	   	</div>
  	</div>
 	</div>
  ...
  <ul class="list-group">
    ...
    <a href ="#" onclick="window.open('ecg_ok.php?id=<?php echo $consulta[$i]['id'] ?>', 'Pagina', 'STATUS=NO, TOOLBAR=NO, LOCATION=NO, DIRECTORIES=NO, RESISABLE=NO, SCROLLBARS=YES, TOP=50, LEFT=50, WIDTH=550, HEIGHT=400');">
